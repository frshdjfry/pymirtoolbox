...
class _PathInitializer(object):
    ...
    def set_interpreter_version(self):
        ...
        if _version in self.SUPPORTED_PYTHON_VERSIONS:
            ...
            warnings.warn(
                'Python versions 3.9, 3.10, 3.11, and 3.12 are supported, but your version of Python is %s' % version_with_dot)
            self.interpreter_version = _version
        else:
    ...
    def update_paths(self):
        ...
        if self.is_windows:
            os.environ[self.path_var] = self.extern_bin_dir + os.pathsep + self.bin_dir + os.pathsep + os.environ[
                self.path_var]
